...
def prepare_vejman_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[
        df["hast_generel_hast"].notna()
    ]  # drop rows where hast_generel_hast is None
    df = df[
        df["hast_gaeldende_hast"].notna()
    ]  # drop rows where hast_gaeldende_hast is None
...
def add_vejman_features(
    ...
    vejman_features = vejman_features.loc[
        vejman_features["hast_senest_rettet"] < f"{dataframe_year+1}/01/01 00:00:00"
    ]
...
def main():
    ...
    for pkl_file, year, outfile in inputs:
        ...
        df = add_vejman_features(df, year, vejman_pkl)
